...
const REAL_BUSINESSES = [
  {
    ...
    logoUrl:
      'https://ui-avatars.com/api/?name=Vefa+Spor&background=0D8ABC&color=fff',
  ...
  },
  {
    ...
    rating: 4.6,
    coverImageUrl:
      'https://images.unsplash.com/photo-1570498839593-e565b39455fc?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80',
    logoUrl:
      'https://ui-avatars.com/api/?name=Yeldegirmeni+Spor&background=F39C12&color=fff',
  ...
  },
  {
    ...
    rating: 4.7,
    coverImageUrl:
      'https://images.unsplash.com/photo-1624880357913-a8539238245b?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80',
    logoUrl:
      'https://ui-avatars.com/api/?name=Cilekli+Tesisleri&background=000&color=fff',
    facilities: [
      'Duş',
      'Otopark',
      'Kafeterya',
...
async function bootstrap() {
  ...
  for (const businessData of REAL_BUSINESSES) {
    ...
    if (existingBusiness) {
      console.log(
        `⚠️ Business already exists: ${businessData.name} - Updating...`,
      );
    ...
    if (pitchesToDelete.length > 0) {
      console.log(
        `   🗑️  Removing ${pitchesToDelete.length} old pitch(es): ${pitchesToDelete.map((p) => p.name).join(', ')}`,
      );
    ...
    for (const pitchData of businessData.pitches) {
      ...
      if (existingPitch) {
        ...
        console.log(
          `   🔸 Pitch Created: ${pitch.name} (${pitch.pricePerHour} TL)`,
        );
      }
    }
  }
