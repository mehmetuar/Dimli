...
async function bootstrap() {
    const app = await NestFactory.createApplicationContext(SeedBusinessOwnerModule);
    ...
    if (!businesses || businesses.length === 0) {
        console.log('❌ No businesses found! Please run seed-real-businesses first.');
        await app.close();
        return;
    }
    ...
    for (const business of businesses) {
        ...
        try {
            ...
            // Create new owner
            const owner = await businessOwnerService.create({
                email: email,
