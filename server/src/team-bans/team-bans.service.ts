...
export class TeamBansService {
  ...
  async isBanned(teamId: string, userId: string): Promise<boolean> {
    const ban = await this.teamBansRepository.findOne({
      where: { teamId, userId },
    });
    return !!ban;
  }
  ...
  async banPlayer(teamId: string, userId: string): Promise<void> {
    const existing = await this.teamBansRepository.findOne({
      where: { teamId, userId },
    });
    if (!existing) {
      await this.teamBansRepository.save(
        this.teamBansRepository.create({ teamId, userId }),
      );
    }
  }
