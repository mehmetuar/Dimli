...
export class BusinessOwnerService {
  ...
  async getDashboardSlots(ownerId: string, dateStr: string) {
    ...
    for (const pitch of pitches) {
      ...
      const isClosed =
        ...
        (pitch.closedDays && pitch.closedDays.includes(dayName));
      if (isClosed) {
        slotsResponse.push({
          pitchId: pitch.id,
  ...
  async deleteAccount(
    ...
    if (!isMatch)
      throw new BadRequestException('Şifre yanlış. Lütfen tekrar deneyin.');

    const now = new Date();

    if (
      owner.business &&
      owner.business.pitches &&
      owner.business.pitches.length > 0
    ) {
      ...
      if (futureReservationsCount > 0) {
        throw new BadRequestException(
          'İleri tarihli kesinleşmiş rezervasyonunuz bulunduğu için hesabınızı şu an silemezsiniz.',
        );
      }
    }

    const queryRunner =
      this.businessOwnerRepository.manager.connection.createQueryRunner();
      ...
      if (owner.business) {
        ...
        if (owner.business.pitches && owner.business.pitches.length > 0) {
          ...
          await queryRunner.query(
            'UPDATE team SET home_pitch_id = NULL WHERE home_pitch_id = ANY($1)',
            [pitchIds],
          );
        }
        ...
        await queryRunner.query(
          'UPDATE team SET home_business_id = NULL WHERE home_business_id = $1',
          [owner.business.id],
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
          'UPDATE business_owner SET "businessId" = NULL WHERE id = $1',
          [owner.id],
        );
      }
