...
import { MOCK_ALL_USERS, CURRENT_USER } from '../constants';
...
export const AddPlayerModal: React.FC<Props> = ({ isOpen, onClose, currentRosterIds }) => {
  ...
  const handleSearch = (term: string) => {
    ...
    // Filter users: Match name, NOT me, NOT already in roster
    const results = MOCK_ALL_USERS.filter(u => 
        u.name.toLowerCase().includes(term.toLowerCase()) && 
        u.id !== CURRENT_USER.id &&
        !currentRosterIds.includes(u.id)
    );
  ...
  const copyInviteLink = () => {
      navigator.clipboard.writeText("https://sahapro.app/invite/team/t1");
