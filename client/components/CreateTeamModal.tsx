...
import { SkillLevel, Team } from '../types';
...
export const CreateTeamModal: React.FC<Props> = ({ isOpen, onClose, onCreate }) => {
  ...
  const colors = [
    { class: 'bg-red-600', label: 'Kırmızı' },
    { class: 'bg-blue-600', label: 'Mavi' },
    { class: 'bg-green-600', label: 'Yeşil' },
    { class: 'bg-yellow-500', label: 'Sarı' },
    { class: 'bg-purple-600', label: 'Mor' },
    { class: 'bg-slate-900', label: 'Siyah' },
    { class: 'bg-orange-500', label: 'Turuncu' },
    { class: 'bg-teal-500', label: 'Turkuaz' },
  ];
  ...
  const handleSubmit = () => {
    ...
    const newTeam: Partial<Team> = {
      ...
      wins: 0,
      losses: 0,
      logoUrl: `https://ui-avatars.com/api/?name=${name}&background=random&color=fff&size=200`, // Auto generate avatar
    };
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center px-4 bg-black/90 backdrop-blur-sm animate-fade-in">
      <div className="bg-slate-800 w-full max-w-lg rounded-3xl border border-slate-700 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
            ...
            <div>
                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block flex items-center gap-2">
                    <Palette className="w-4 h-4" /> Forma Rengi
                </label>
