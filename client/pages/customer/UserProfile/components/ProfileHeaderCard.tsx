...
interface ProfileHeaderCardProps {
    ...
    /** GPS alınıyor veya sunucu senkronu sürüyor — KONUM hücresinde sessiz pulse göstergesi */
                    ...
                    <div className="relative z-10 p-6 flex flex-col items-center">
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full">
                            ...
                            <div className="bg-slate-800/50 p-2 sm:p-3 rounded-xl border border-slate-700 flex flex-col items-center justify-center gap-0.5 sm:gap-1 relative group">
                                ...
                                <span className={`text-white font-sport text-base sm:text-lg font-bold truncate max-w-[100px] xs:max-w-[120px] sm:max-w-full text-center ${isLocationUpdating ? 'animate-pulse' : ''}`}>
                                    {liveLocation || currentUser.location || (isLocationUpdating ? 'Konum alınıyor…' : 'İstanbul')}
                                </span>
                                <button onClick={() => handleUpdateLocation(false)} className="absolute top-1 right-1 text-turf-500 hover:text-white transition-colors">
                                    <MapPin className={`w-3 h-3 ${isLocationUpdating ? 'animate-pulse' : ''}`} />
                                </button>
                            </div>
