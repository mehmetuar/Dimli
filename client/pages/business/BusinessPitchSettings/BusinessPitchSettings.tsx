                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <Power className={`w-4 h-4 flex-shrink-0 ${formData.isActive ? 'text-emerald-400' : 'text-slate-500'}`} />
                        ...
                        >
                            <span className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow-md transition-transform duration-200 ${formData.isActive ? 'translate-x-7' : 'translate-x-0'}`} />
                        </button>
                    </div>
                    {!formData.isActive && (
                        <div className="mt-3 p-3 bg-red-500/10 border border-red-500/25 rounded-xl">
                            <p className="text-[clamp(10px,2.8vw,12px)] text-red-400 font-medium">
                ...
                <div className="bg-slate-800/70 rounded-2xl border border-slate-700/60 overflow-hidden"
                    ...
                    <div className="px-4 py-3.5 border-b border-slate-700/50"
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {closedDaysSuccess && (
                                <span className="text-[clamp(10px,2.8vw,12px)] text-emerald-400 font-bold">✓ Kaydedildi</span>
                            )}
                            {savingClosedDays && (
                                <span className="text-[clamp(10px,2.8vw,12px)] text-slate-400">Kaydediliyor...</span>
                            )}
                        </div>
                        <p className="text-[clamp(9px,2.5vw,11px)] text-slate-400 mt-1 ml-9">
                            Haftalık kapalı günleri seçin
                        </p>
                    </div>
                    <div className="p-4">
                        ...
                        {formData.closedDays.length > 0 && (
                            <p className="text-[clamp(9px,2.5vw,11px)] text-slate-500 mt-3">
                                Kapalı: {formData.closedDays.map((d: string) => DAYS.find(x => x.value === d)?.label).filter(Boolean).join(', ')}
                            </p>
                        )}
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="bg-slate-800/70 rounded-2xl border border-slate-700/60 overflow-hidden"
                        ...
                        {pendingPhotoUrl ? (
                            ...
                            <>
                                {formData.imageUrl ? (
                                    <img src={formData.imageUrl} alt={formData.name} className="w-full aspect-video object-cover" />
                                ) : (
